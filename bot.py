...
# Глобальная переменная для хранения групп медиа
media_groups = {}
...
# === ОБРАБОТКА ГРУПП МЕДИА ===
def process_media_group(media_group_id):
    """Обрабатывает группу медиа после сбора всех фото"""
...
📊 <b>Статус:</b> {status}"""
        ...
        elif call.data.startswith('publish_normal_'):
